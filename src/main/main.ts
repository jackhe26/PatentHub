...
// ===== DOMMatrix polyfill for pdf-parse - 必须在所有 import 之前执行 =====
// 使用强制方式设置全局变量，确保在任何模块加载前 polyfill 就存在
...
// ========== Portable版本用户数据目录配置 ==========
// 修复Portable版本PDF预览显示异常问题
// 当运行Portable版本时，Chromium需要用户数据目录才能正确初始化PDF Viewer
if (process.platform === 'win32') {
  // 检测是否是Portable版本（electron-builder会在Portable版本设置此环境变量）
  const portableDir = process.env.PORTABLE_EXECUTABLE_DIR
  if (portableDir) {
    // 将用户数据目录设置为Portable版本同文件夹下的userData目录
    const userDataPath = path.join(portableDir, 'userData')
    app.setPath('userData', userDataPath)
    console.log(`[Portable Mode] User data directory set to: ${userDataPath}`)
  }
}
...
import { registerIPCHandlers as registerPdfTranslateIPCHandlers } from './pdf-translate/index.js'
...
if (!gotTheLock) {
  ...
  app
    ...
    .then(async () => {
      ...
      registerShortcuts()
      proxy.init()
      registerPdfTranslateIPCHandlers()
      app.on('will-quit', () => {
        try {
          unregisterShortcuts()
        } catch (e) {
...
ipcMain.handle('parseFileLocally', async (event, dataJSON: string) => {
  ...
  try {
    ...
    // 直接使用错误消息，因为 file-parser.ts 已经抛出了详细的错误信息
    // 格式为 "解析器名称 解析失败: 具体原因"
    let detailedReason = errorMessage
    
    // 如果错误消息不够详细，添加额外的分析
    if (!errorMessage.includes('解析失败') && !errorMessage.includes('Failed') && !errorMessage.includes('无法加载模块')) {
      // 检查是否是特定的库加载失败
      if (errorMessage.includes('Cannot find module') || errorMessage.includes('MODULE_NOT_FOUND')) {
        if (errorMessage.includes('officeparser')) {
          detailedReason = 'officeparser 库未正确加载，请检查打包配置'
        } else if (errorMessage.includes('pdf-parse') || errorMessage.includes('pdf-parse-2')) {
          detailedReason = 'pdf-parse 库未正确加载，请检查打包配置'
        } else if (errorMessage.includes('epub')) {
          detailedReason = 'epub 库未正确加载，请检查打包配置'
        } else {
          detailedReason = `缺少依赖模块: ${errorMessage}`
        }
      } else if (errorMessage.includes('ENOENT') || errorMessage.includes('no such file')) {
        detailedReason = '文件不存在或路径错误'
      } else if (errorMessage.includes('permission') || errorMessage.includes('EACCES')) {
        detailedReason = '没有文件读取权限'
      } else if (errorMessage.includes('out of memory') || errorMessage.includes('heap')) {
        detailedReason = '文件过大，内存不足'
      } else if (errorMessage.includes('Invalid') || errorMessage.includes('corrupt')) {
        detailedReason = '文件格式无效或已损坏'
      }
    }
    
    // 添加更多调试信息到错误消息中（帮助用户排查问题）
    const debugInfo = `\n\n【调试信息】\n- app.getAppPath(): ${app.getAppPath()}\n- app.isPackaged: ${app.isPackaged}\n- PORTABLE_EXECUTABLE_DIR: ${process.env.PORTABLE_EXECUTABLE_DIR || '未设置'}\n- process.resourcesPath: ${process.resourcesPath}`
    
    // 如果是模块加载失败，添加调试信息
    if (errorMessage.includes('无法加载模块') || errorMessage.includes('Failed to load')) {
      detailedReason = detailedReason + debugInfo
    }
    
    console.error('[parseFileLocally] Error:', error.message, '\nStack:', errorStack)
