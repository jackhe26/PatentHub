...
export default function Sidebar() {
  ...
  const handleOpenPDFTranslate = useCallback(async () => {
    // 创建新的 PDF 翻译 Session
    const newSession = await createSession({
      type: 'pdf-translate',
      name: 'New PDF Translation',
      messages: [],
    })
    
    // 导航到 PDF 翻译页面，传递 sessionId
    navigate({ to: '/pdf-translate', search: { sessionId: newSession.id } })
    if (isSmallScreen) {
      setShowSidebar(false)
    }
    trackingEvent('open_pdf_translate', { event_category: 'user' })
  }, [isSmallScreen, setShowSidebar, navigate])
      ...
      >
        ...
        <Flex align="center" justify="space-between" px="md" py="sm">
          <Flex align="center" gap="sm">
            <Flex
              align="center"
              gap="sm"
              onClick={() => platform.openLink('http://www.cnipa-sc.com/')}
        ...
        <Stack gap={0} px="xs" pb="xs">
          ...
          <Stack gap="xs" pt="xs" mb="xs">
            ...
            <Button variant="light" fullWidth onClick={handleOpenPDFTranslate}>
              <ScalableIcon icon={IconFileTypeJs} className="mr-2" />
              {t('PDF Translate')}
            </Button>
          </Stack>
          <NavLink
