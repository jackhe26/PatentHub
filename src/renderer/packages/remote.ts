...
// 缓存精选搭子数据，用于离线使用
let cachedCopilots: CopilotDetail[] | null = null
...
export async function listCopilots(lang: string) {
  ...
  try {
    // 使用本地服务器（专利搭子）- 改为 localhost
    const res = await ofetch<Response>('http://localhost:3000/api/copilots/list', {
      method: 'POST',
      retry: 3,
      body: { lang },
    })
    // 成功获取后缓存数据
    cachedCopilots = res.data
    return res.data
  } catch (error) {
    // 如果请求失败，返回缓存的数据（离线模式）
    log.warn('获取精选搭子失败，使用缓存数据', error)
    if (cachedCopilots && cachedCopilots.length > 0) {
      return cachedCopilots
    }
    // 如果没有缓存，抛出错误
    throw error
  }
