...
export default class DesktopPlatform implements Platform {
  ...
  async parseFileLocally(file: File): Promise<{ key?: string; isSupported: boolean; error?: string }> {
    let result: { text: string; isSupported: boolean; error?: string; rawError?: string }
    if (!file.path) {
    ...
    if (!result.isSupported) {
      // 返回详细的错误信息
      const errorMsg = result.error || result.rawError || '文件解析失败'
      console.error('[parseFileLocally] Parsing failed:', errorMsg)
      return { isSupported: false, error: errorMsg }
    }
