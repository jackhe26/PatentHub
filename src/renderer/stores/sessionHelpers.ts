...
export async function preprocessFile(
  ...
  content: string
  storageKey: string
  filePath?: string // Electron 特有：文件真实路径
  tokenCountMap?: Record<string, number>
  lineCount?: number
  byteLength?: number
  error?: string
}> {
  // Get file path from Electron's File object (if available)
...
export function constructUserMessage(
  ...
  if (preprocessedFiles.length > 0) {
    msg.files = preprocessedFiles.map((f) => ({
      ...
      url: f.filePath, // 存储 Electron 文件路径供 PDF 预览使用
