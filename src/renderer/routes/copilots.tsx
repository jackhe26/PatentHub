...
function Copilots() {
  ...
  const [showRemoteCopilots, setShowRemoteCopilots] = useState(false) // 默认关闭Chatbox精选
  ...
  useEffect(() => {
    if (!open) {
      ...
      trackingEvent('copilots_window', { event_category: 'screen_view' })
    }
  }, [open])
          ...
          <>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                <MantineSwitch
                  checked={showRemoteCopilots}
                  onChange={(event) => setShowRemoteCopilots(event.currentTarget.checked)}
                  label={t('Show PatentHub Featured')}
                />
              </Box>
            </Box>
...
function MiniItem(props: MiniItemProps) {
  ...
  const avatarEmoji = emojiMatch ? emojiMatch[1] : null
